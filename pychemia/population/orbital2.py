...
def params2dmatpawu(params, ndim):
    ...
    if 'num_matrices' in params:
        ...
        num_matrices = len(params['D'])

    ret = np.zeros((num_matrices, ndim, ndim))

    for i in range(num_matrices):
        eigval = np.diag(params['O'][i])
        for j in range(ndim):
            if eigval[j, j] == 0:
                eigval[j, j] += params['D'][i][j]
            else:
                eigval[j, j] -= params['D'][i][j]
        rotation = gea_orthogonal_from_angles(params['E'][i])
...
def dmatpawu2params(dmatpawu, ndim):
    ...
    return {'O': occupations, 'D': deltas, 'E': euler_angles, 'num_matrices': num_matrices}
