...
class Convergence:
    ...
    def is_converge(self):
        ...
        if (
            len(energies) > 2
            and abs(max(energies[-3:]) - min(energies[-3:])) >= self.energy_tolerance
        ):
            return False
        else:
            return True
    ...
    def _convergence_plot(self, variable, xlabel, title, figname, annotate):
        ...
        plt.fill_between(
            xlims, [low_dy, low_dy], [sup_dy, sup_dy], color="0.9", alpha=0.5
        )
        assert self.convergence_info is not None
        for idata in self.convergence_info:
            plt.annotate(
                s=str(idata[annotate]),
                xy=(idata[variable], idata["free_energy"]),
                size=10,
            )
    ...
    def _convergence_save(self, filename):
        ...
        wf = open(filename, "w")
        json.dump(
            self.convergence_info, wf, sort_keys=True, indent=4, separators=(",", ": ")
        )
        wf.close()
...
class ConvergenceCutOffEnergy(Task, Convergence):
    def __init__(
        ...
        if extra_vars is not None:
            ...
            self.extra_vars = {}
        if kpoints is None:
            kp = KPoints.optimized_grid(
                self.structure.lattice, kp_density=1e4, force_odd=True
            )
            self.kpoints = kp
        else:
    ...
    def run(self, nparal=4):
        ...
        while True:
            ...
            vj.input_variables.set_encut(
                ENCUT=x, POTCAR=self.workdir + os.sep + "POTCAR"
            )
            vj.input_variables.variables["NBANDS"] = int(
                nparal * ((30 + self.structure.valence_electrons()) / nparal + 1)
            )
            ...
            vj.get_outputs()
            free_energy = (
                vj.outcar.final_data["energy"]["free_energy"] / self.structure.natom
            )
            print("encut= %7.3f  free_energy: %9.6f" % (encut, free_energy))
            self.convergence_info.append(
                {"free_energy": free_energy, "encut": encut, "factor": x}
            )
    ...
    def plot(self, filedir=None, file_format="pdf"):
        ...
        return self._convergence_plot(
            variable="encut",
            xlabel="ENCUT",
            title="ENCUT Convergence",
            figname=figname,
            annotate="encut",
        )
    ...
    def report(self, file_format="html"):
        ...
        html = element_maker.html(
            E.head(E.title("VASP Energy cut-off Convergence")),
            E.body(
                E.h1("VASP Energy cut-off Convergence"),
                E.h2("Structure"),
                E.pre(str(self.structure)),
                E.h2("Convergence"),
                E.p(
                    E.img(
                        src="convergence.jpg", width="800", height="600", alt="Forces"
                    )
                ),
            ),
        )
...
class ConvergenceKPointGrid(Task, Convergence):
    ...
    def run(self, nparal=4):
        ...
        while True:
            density = n ** 3
            kp = KPoints.optimized_grid(
                self.structure.lattice, kp_density=density, force_odd=True
            )
            ...
            if np.sum(grid) != np.sum(kp.grid):
                ...
                print(
                    "kp_density= %10d kp_grid= %15s free_energy= %9.6f"
                    % (density, grid, energy)
                )
                self.convergence_info.append(
                    {
    ...
    def plot(self, filedir=None, file_format="pdf"):
        ...
        return self._convergence_plot(
            variable="kp_density",
            xlabel="K-points density",
            title="KPOINTS Convergence",
            figname=figname,
            annotate="kp_grid",
        )
    ...
    def best_kpoints(self):
        ...
        if not self.is_converge:
            ...
            kp = KPoints.optimized_grid(
                ...
                kp_density=self.convergence_info[-3]["kp_density"],
                force_odd=True,
            )
            return kp
    ...
    def report(self, file_format="html"):
        ...
        html = element_maker.html(
            E.head(E.title("VASP K-point grid Convergence")),
            E.body(
                E.h1("VASP K-point grid Convergence"),
                E.h2("Structure"),
                E.pre(str(self.structure)),
                E.h2("Convergence"),
                E.p(
